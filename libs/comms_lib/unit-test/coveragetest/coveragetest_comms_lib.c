...
/*
** File: coveragetest_comms_lib.c
**
** Purpose:
** Coverage Unit Test cases for the COMMS library
**
** Notes:
** This implements various test cases to exercise all code
** paths through all functions defined in the COMMS library.
**
...
#include "comms_lib_coveragetest_common.h"
...
char UT_TESTBUFFER[] = "COMMSLIB_UT";
...
} COMMS_LIB_Function_TestState_t;
...
static int32 UT_printf_hook(void *UserObj, int32 StubRetcode, uint32 CallCount, const UT_StubContext_t *Context,
                            va_list va)
{
    COMMS_LIB_Function_TestState_t *State  = UserObj;
     ...
     */
    if (Context->ArgCount > 0 && strcmp(string, "COMMS_LIB_Function called, buffer=\'%s\'\n") == 0)
    {
...
void Test_COMMS_LIB_Init(void)
{
    ...
    UT_TEST_FUNCTION_RC(COMMS_LIB_Init(), CFE_SUCCESS);
     ...
     */
    UtAssert_StrCmp(UT_TESTBUFFER, COMMS_LIB_Buffer, "Internal buffer content valid");
    ...
    UT_TEST_FUNCTION_RC(COMMS_LIB_Init(), CFE_STATUS_NOT_IMPLEMENTED);
}

void Test_COMMS_LIB_Function(void)
{
    /*
     * Test Case For:
     * void COMMS_LIB_Function( void )
     */
    COMMS_LIB_Function_TestState_t state;
...
{
    ADD_TEST(COMMS_LIB_Init);
    ADD_TEST(COMMS_LIB_Function);
}
